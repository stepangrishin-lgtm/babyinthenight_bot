...
def recall_girl(memory):
    ...
    phrases = [
        ...
        f"а Леха бы оценил на {girl['score']}/10",
...
def make_reply():
    ...
    verdicts = [
        ...
        "достойный уровень (возможно даже лазерный)",
        "ночная смена одобряет",
        "батя её явно не в восторге от того, чем она занимается",
        "комиссия стерла ладони",
        "ебут же люди...",
        "4 лайка и я скину с ней фулл",
        "это ж Саня из второго подъезда",
        "в Химках её видал",
    ]

    templates = [
        "Оценка {score}/10. Вердикт: {verdict}.",
        "{score}/10. Ну что тут скажешь: {verdict}.",
        "Итог: {score}/10 — {verdict}.",
        "Оценка {score}/10. Заявляю как эксперт: {verdict}.",
        "Оценка {score}/10. Могу сказать только одно: {verdict}.",
    ]
